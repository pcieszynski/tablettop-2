...
public class LeftHandResource {
     ...
     *
     * @param leftHand the leftHand to create
     * @return the ResponseEntity with status 201 (Created) and with body the new leftHand, or with status 400 (Bad Request) if the leftHand has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/left-hands")
    @Timed
    public ResponseEntity<LeftHand> createLeftHand(@RequestBody LeftHand leftHand) throws URISyntaxException {
        log.debug("REST request to save LeftHand : {}", leftHand);
        if (leftHand.getId() != null) {
            throw new BadRequestAlertException("A new leftHand cannot already have an ID", ENTITY_NAME, "idexists");
        }
        LeftHand result = leftHandService.save(leftHand);
     ...
     *
     * @param leftHand the leftHand to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated leftHand,
     * or with status 400 (Bad Request) if the leftHand is not valid,
     * or with status 500 (Internal Server Error) if the leftHand couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/left-hands")
    @Timed
    public ResponseEntity<LeftHand> updateLeftHand(@RequestBody LeftHand leftHand) throws URISyntaxException {
        log.debug("REST request to update LeftHand : {}", leftHand);
        if (leftHand.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        LeftHand result = leftHandService.save(leftHand);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, leftHand.getId().toString()))
            .body(result);
    }
    ...
    @Timed
    public List<LeftHand> getAllLeftHands(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
     ...
     *
     * @param id the id of the leftHand to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the leftHand, or with status 404 (Not Found)
     */
    @GetMapping("/left-hands/{id}")
    @Timed
    public ResponseEntity<LeftHand> getLeftHand(@PathVariable Long id) {
        log.debug("REST request to get LeftHand : {}", id);
        Optional<LeftHand> leftHand = leftHandService.findOne(id);
        return ResponseUtil.wrapOrNotFound(leftHand);
    }

    /**
     * DELETE  /left-hands/:id : delete the "id" leftHand.
     *
     * @param id the id of the leftHand to delete
